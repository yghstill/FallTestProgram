...
public class XCRoundImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (bmp == null || bmp.isRecycled()) {
			...
			if (null != drawable) {
				...
				if (mType == TYPE_CIRCLE) {// 如果是圆�?
					scale = getWidth() * 1.0F / Math.min(dwidth, dheight);
				} else if (mType == TYPE_ROUND || mType == TYPE_OVAL) {// 如果是圆角矩形或椭圆
																		// 如果图片的宽或�?�高与view的宽高不匹配，计算出�?要缩放的比例�?
																		// 缩放后的图片的宽高，�?定要大于我们view的宽高；�?以我们这里取大�?�；
	...
	/**
	 * 因为使用了缓存技术，�?以需要在invalidate中做些回收释放资源的处理
	 */
	@Override
